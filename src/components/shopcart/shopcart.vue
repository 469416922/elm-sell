  ...
  <div class="shopcart">
    ...
    <transition name="fold">
      <div class="shopcart-list" v-show="listShow">
        ...
        <div class="list-count" ref="listcountent">
          <ul>
...
export default {
  ...
  methods: {
    ...
    drop (el){
      // console.log(el) // 点击加号看是否输出成功 获取到cartcontrol中的dom
    ...
    beforeEnter: function (el) {
      ...
      while (count--) {
        ...
        if (ball.show) {
          ...
          // console.log('x:y' + rect.left + ':' + rect.right)
          // el.style.display = ''
          // el.style.webkitTransform = `translate3d(0,${y}px, 0)`
          // el.style.transform = `translate3d(0,${y}px, 0)`
          let inner = el.getElementsByClassName('inner-hook')[0]
          inner.style.webkitTransform = `translate3d(${rect.left}px,${rect.top}px, 0)`
          // inner.style.webkitTransform = 'translate3d(493px,302px, 0)'
          inner.style.transform = `translate3d(${rect.x}px,${rect.y}px, 0)`
        }
      }
    },
    ...
    // vue官网实例是leave 但是这里不执行换成了v-on:after-enter
    leave: function (el, done) {
      setTimeout(() => { // 立即执行会看不到动画不知道怎么解决就用了定时器
