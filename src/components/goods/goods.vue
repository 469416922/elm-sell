...
<div class="goods">
  ...
  ></shopCart>
  <foodcontrol :food="selectedFood" ref="foodcontrol" v-on:cart-add="cartAdd"></foodcontrol>
</div>
</template>
...
import foodcontrol from '@/components/food/food'
const ERR_OK = 0
export default {
  props: {
    seller: {
  ...
  methods: {
    selectFood (food){
      ...
      this.$refs.foodcontrol.show() // this.$refs['foodcontrol'].show()
    },
    cartAdd (el){
    ...
    _initScroll (){
      ...
      this.foodsScroll = new BScroll(this.$refs.foodsWrapper, {
        click: true,
        probeType: 3 // 探针3代表实时返回当前位置
      })
      // 监听滚动 获取当前所在滚动位置 赋值给scrollY
      this.foodsScroll.on('scroll', (pos) => {
        this.scrollY = Math.abs(Math.round(pos.y))
      })
    },
    _calculateHeight (){
      // 获取全部的food-list-hook 的DOM集合
      let foodList = this.$refs.foodsWrapper.getElementsByClassName('food-list-hook')
      let height = 0
      // 给listHeight添加第一高度0
      this.listHight.push(height)
      // 获取对应DOM 的高度 依次存进listHight
